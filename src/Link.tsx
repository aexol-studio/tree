...
// };
const buildQuadraticPath = (x1: number, y1: number, x2: number, y2: number, cornerRadius: number): string => {
  let centerX = (x1 + x2) / 2.0;
  ...
  return `M ${x1} ${y1} ${centerX-crx} ${y1} Q ${centerX} ${y1} ${centerX} ${y1+cry} L ${centerX} ${y2-cry} Q ${centerX} ${y2} ${centerX+crx} ${y2} L ${x2} ${y2}`;
};
