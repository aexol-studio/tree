...
import { RoundedRectangle, RoundedRectangleStroke } from './Draw/RoundedRectangle';
...
export class NodeRenderer {
  ...
  render = ({
    ...
    if (!node.hidden) {
      const {
        colors,
        node: { width: nodeWidth, height, nameSize, typeSize, options, radius, typeYGap, padding },
        description,
      } = this.theme;

      const width = Math.max(nodeWidth, this.context.measureText(node.name).width + padding * 2);

      this.context.fillStyle = colors.node.backgrounds?.[node.type] || colors.node.background;
      ...
      this.context.fillStyle = colors.node.types?.[node.type] || colors.node.type;
      ...
      if (node.options) {
        ...
        node.options.forEach((o) => {
          ...
          this.context.fillText(o, node.x + xCounter, node.y + height + options.fontSize + options.yGap, width);
      ...
      if (node.name) {
        ...
        this.context.fillText(node.name, node.x + width / 2.0, node.y + height / 2.0);
      }
      ...
      if (isSelected) {
        ...
        if (node.description) {
          ...
          const numberOfLines = Math.ceil(
            this.context.measureText(node.description).width / (dWidth - description.paddingHorizontal * 2),
          );
          const lineHeight = description.fontSize * 1.5;
          const textHeight = lineHeight * numberOfLines;
          const y = node.y - textHeight - description.paddingVertical * 2 - description.triangleDistance;
          RoundedRectangle(this.context, {
            width: dWidth,
